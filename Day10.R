...
reg <- lm(wage ~ educ, data = wage1)        # fit the OLS line:  lm(wage ~ educ, data = wage1) dep. on left
reg$coefficients     # look: the two estimates  (-0.90 and 0.54, same as Day 9) gives b0 and b1 hat
...
uhat <- yhat - 9   # (b) residual = actual wage (9.0) - fitted
## (c) Over- or under-predicted? (comment)  ANSWER: underpredicted